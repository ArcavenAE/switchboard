...
# create-pkg.sh - Create a signed macOS pkg installer
...
cp "$BINARY_PATH" "$STAGING_DIR/usr/local/bin/switchboard"
chmod +x "$STAGING_DIR/usr/local/bin/switchboard"

pkgbuild --root "$STAGING_DIR" \
  --identifier com.arcaven.switchboard \
  --version "$VERSION" \
  --install-location / \
